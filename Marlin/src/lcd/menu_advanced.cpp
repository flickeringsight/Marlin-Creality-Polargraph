...
void menu_advanced_settings() {
  ...
  BACK_ITEM(MSG_CONFIGURATION);
      ...
      if (!is_busy) {
        ...
        EDIT_ITEM_FAST(float51sign, MSG_DRAW_MIN_X, &draw_area_min.x, X_MIN_POS, draw_area_max.x - 100); // M665 L
        EDIT_ITEM_FAST(float51sign, MSG_DRAW_MAX_X, &draw_area_max.x, draw_area_min.x + 100, X_MAX_POS); // M665 R
        EDIT_ITEM_FAST(float51sign, MSG_DRAW_MIN_Y, &draw_area_min.y, Y_MIN_POS, draw_area_max.y - 100); // M665 T
        EDIT_ITEM_FAST(float51sign, MSG_DRAW_MAX_Y, &draw_area_max.y, draw_area_min.y + 100, Y_MAX_POS); // M665 B
        EDIT_ITEM_FAST(float51sign, MSG_MAX_BELT_LEN, &polargraph_max_belt_len, 500, 2500);             // M665 H
      }
    #endif
